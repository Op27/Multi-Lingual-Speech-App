document.addEventListener('DOMContentLoaded', function() {
    ...
    function enqueueAudio(blob) {
        ...
        if (!currentAudio || currentAudio.ended || currentAudio.paused) {
            // Only enqueue if there's no current audio, or if it's ended or paused
            ...
            console.log("Current audio is still playing. New audio will not be enqueued.");
        }
    }    
    ...
    startRecordBtn.addEventListener('click', function() {
        if (!isRecording) {
            console.log("Already recording or processing. Please wait.");
            return; 
        }
        if (!isRecording) {
            startRecording();
    ...
    function sendAudioToServer() {
        isRecording = false; // Ensure the recording state is off when sending data
        ...
        .then(data => {
            ...
            isSynthesizingSpeech = false; // Reset the synthesizing flag after playback starts
        }).catch(error => {
            console.error('Error:', error);
            processingIndicator.style.display = 'none';
            isSynthesizingSpeech = false; // Reset the synthesizing flag in case of error
        });
    }    
