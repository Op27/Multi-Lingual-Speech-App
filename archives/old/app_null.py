...
@app.route('/test_static_audio', methods=['GET'])
def test_static_audio():
    file_path = 'C:/Users/owner/Documents/my_project/voice-translation/mono.wav'
    ...
    try:
        ...
        if response.results:
            ...
            return Response("No transcription results", status=500, mimetype='text/plain')
    except Exception as e:
        logging.error(f"Error processing static audio: {str(e)}")
        return Response(f"Error processing static audio: {str(e)}", status=500, mimetype='text/plain')
...
def process_audio():
    ...
    try:
        ...
        language_code="en-US",
        audio_channel_count=1,
        enable_automatic_punctuation=True  # Optional, for better punctuation in transcripts
    )

        response = speech_client.recognize(config=config, audio=audio)
        logging.debug(f"Google Speech API Response: {response}")
